#!/bin/bash
# Claude Code hook — sends session events to AgentRGB Electron app

INPUT=$(cat)
APP_DIR="$HOME/Documents/agent-rgb"
LOG="/tmp/agent-rgb-hook.log"
...
echo "[$(date '+%H:%M:%S')] terminal_pid=$AGENT_RGB_TERM_PID name=$AGENT_RGB_TERM_NAME" >> "$LOG"
...
# Wake up AgentRGB if not running
