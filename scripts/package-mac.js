...
const outputApp = path.join(distDir, 'AgentLight.app')
...
const appBinary = path.join(macosDir, 'AgentLight')
...
fs.copyFileSync(iconFile, path.join(resourcesDir, 'AgentLight.icns'))

fs.writeFileSync(path.join(appDir, 'package.json'), JSON.stringify({
  name: 'agent-light',
  productName: 'AgentLight',
  version: '1.0.0',
  main: 'main.js',
}, null, 2))
...
setPlist('CFBundleName', 'AgentLight')
setPlist('CFBundleDisplayName', 'AgentLight')
setPlist('CFBundleIdentifier', 'com.agentlight.app')
setPlist('CFBundleIconFile', 'AgentLight.icns')

if (fs.existsSync(electronBinary)) {
  fs.renameSync(electronBinary, appBinary)
  setPlist('CFBundleExecutable', 'AgentLight')
}
